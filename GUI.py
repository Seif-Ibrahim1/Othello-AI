...
class OthelloGUI:
    ...
    def start_game(self):
        ...
        self.controller.color_and_difficulty(color_choice, difficulty_choice)
        ...
        if not self.controller.is_player_turn:
    ...
    def make_move(self, row, col):
        if self.controller.make_user_move(row, col):
            self.update_board_display()
    ...
    def computer_play(self):
        if self.controller.make_computer_move():
            self.update_board_display()
            self.check_game_over()
            if not self.controller.switch_to_player():
    ...
    def check_game_over(self):
        if self.controller.check_game_over():
            ...
            winner = self.controller.game.get_winner()
            if winner == "Draw":
                self.show_alert("It's a draw!")
            elif winner == self.controller.current_player.color:
    ...
    def update_board_display(self):
        ...
        for row in range(8):
            for col in range(8):
                if self.controller.game.board[row][col] == "B":
                    self.board_canvas.itemconfig(self.board_buttons[row][col], fill="black")
                elif self.controller.game.board[row][col] == "W":
        ...
        available_moves = self.controller.move.get_available_moves(self.controller.current_player)
        ...
        scores = self.controller.game.get_score()
        if self.controller.current_player.color == "B":
    ...
    def update_info_label(self, message, scores):
        score_text = f"Player: {scores[self.controller.current_player.color]}   Computer: {scores[self.controller.computer_player.color]}"  # Construct score text
